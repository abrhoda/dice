...
func (t token) evaluate() (int, error) {
	...
	case dice:
		...
		// need these delcared beforehand to be in scope outside of `if idx != 0 else` statement.
		count := 1
		var err error
...
func buildAstFromTokens(tokens []token, mbp float64) (*node, error) {
	...
	for {
		...
		eofOrOp, tokens = popToken(tokens)
